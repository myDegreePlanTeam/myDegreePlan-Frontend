  ...
  return (
    ...
    >
      ...
      {isInvalid && validity.missingPrereqs.length > 0 && (
        <div className="mt-1 pt-1 border-t border-red-800 border-opacity-50">
...
export function ElectiveSlot({ slot, isOverlay = false }: ElectiveSlotProps) {
  ...
  const isFilled = !!slot.electiveCode;
  const isCompleted = slot.completed;
  const filledCourse = slot.electiveCode ? courseMap.get(slot.electiveCode) : undefined;
  ...
  const style = transform ? { transform: CSS.Translate.toString(transform) } : undefined;
  const tileClass = isCompleted ? 'tile-completed' : isFilled ? 'tile-valid' : 'tile-unfilled';
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          {isFilled ? (
            filledCourse ? (
              <PrereqTooltip courseCode={slot.electiveCode!} validity={undefined}>
                <div>
                  <span className="font-mono text-[10px] text-accent-blue font-semibold block">
                    {slot.electiveCode}
                  </span>
                  <p className="text-xs text-slate-300 font-medium line-clamp-1">{filledCourse.name}</p>
                </div>
              </PrereqTooltip>
            ) : (
              <span className="font-mono text-[10px] text-accent-blue">{slot.electiveCode}</span>
            )
          ) : (
            <p className="text-xs text-slate-500 italic mt-0.5">
              Drag any course here · {slot.slotCredits ?? 3} cr
            </p>
          )}
        </div>
